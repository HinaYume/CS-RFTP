...
class PragmaVoteDecl final
  ...
  PragmaVoteDecl(TranslationUnitDecl *TU, SourceLocation Loc,
                           size_t ValueStart, SourceLocation StartL = SourceLocation(),
                           SourceLocation RBraL= SourceLocation(), SourceLocation LBraL= SourceLocation())
      : Decl(PragmaVote, TU, Loc), ValueStart(ValueStart), LocStart(StartL), RBraceLoc(RBraL), LBraceLoc(LBraL){}// ,EndRangeLoc(NameInfo.getEndLoc()){}
...
public:
  ...
  StringRef getValue() const { return getTrailingObjects<char>() + ValueStart; }
    ...
    void setVoteType(StringRef T); /*const {   
      char *Buffer = new (getASTContext(), 1) char[T.size() + 1];
      memcpy(Buffer, T.data(), T.size());
      Buffer[T.size()] = '\0';
      VoteType = Buffer; 
...
//std::vector<std::string> getDep(){return Dep;}
void setDep(std::vector<std::string> T);//{Dep = T;}

llvm::ArrayRef<std::string> getDepType() const { return DepType; }
void setDepType(std::vector<std::string> T);//{Dep = T;}

  SourceRange getSourceRange() const override LLVM_READONLY {
    return SourceRange(LocStart, RBraceLoc);
  }
  SourceLocation getLocStart() const LLVM_READONLY { return LocStart; }
  SourceLocation getRBraceLoc() const LLVM_READONLY { return RBraceLoc; }  //{ return RBraceLoc; }
  SourceLocation getLBraceLoc() const LLVM_READONLY { return LBraceLoc; }  //{ return RBraceLoc; }
