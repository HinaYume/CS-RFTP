...
namespace clang {
  ...
  public:
    ...
    // Adding by P
    void VisitPragmaVoteDecl(PragmaVoteDecl *D);
...
// Adding by P
void ASTDeclReader::VisitPragmaVoteDecl(PragmaVoteDecl *D) {
  VisitDecl(D);
  D->setLocation(ReadSourceLocation());
  ...
  D->ValueStart = Name.size() + 1;
  //std::string Value = ReadString();
  memcpy(D->getTrailingObjects<char>() + D->ValueStart, Name.data(),
         Name.size());
  D->getTrailingObjects<char>()[D->ValueStart + Name.size()] = '\0';
...
Decl *ASTReader::ReadDeclRecord(DeclID ID) {
  ...
  case DECL_PRAGMA_DETECT_MISMATCH:
    ...
    break;
  // Adding by P
  case DECL_PRAGMA_VOTE:
    D = PragmaVoteDecl::CreateDeserialized(Context, ID,
                                                     Record.readInt());
    break;
  case DECL_EMPTY:
