...
/// Adding by P
void CodeGenModule::AddVote(StringRef Name, StringRef Value) {
  llvm::SmallString<32> Opt;
  getTargetCodeGenInfo().getVoteOption(Name, Name, Opt);
  auto *MDOpts = llvm::MDString::get(getLLVMContext(), Opt);
  LinkerOptionsMetadata.push_back(llvm::MDNode::get(getLLVMContext(), MDOpts));
}
...
void CodeGenModule::EmitTopLevelDecl(Decl *D) {
  ...
  // Adding by P
  case Decl::PragmaVote: {
    const auto *PDMD = cast<PragmaVoteDecl>(D);
    AddVote(PDMD->getName(), PDMD->getName());//->getValue());
    break;
  }
