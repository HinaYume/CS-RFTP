...
// Adding by P
void PragmaVoteDecl::anchor() { }

PragmaVoteDecl *
PragmaVoteDecl::Create(const ASTContext &C, TranslationUnitDecl *DC,
                                 SourceLocation Loc, StringRef Name){//,
                                 //StringRef Value) {
  size_t ValueStart = Name.size() + 1;
  PragmaVoteDecl *PDMD =
      new (C, DC, additionalSizeToAlloc<char>(ValueStart + Name.size() + 1))// + Value.size() + 1))
          PragmaVoteDecl(DC, Loc, ValueStart);
  memcpy(PDMD->getTrailingObjects<char>(), Name.data(), Name.size());
  PDMD->getTrailingObjects<char>()[Name.size()] = '\0';
  memcpy(PDMD->getTrailingObjects<char>(), Name.data(), Name.size());// + ValueStart);//, Value.data(),
         //Value.size());
  PDMD->getTrailingObjects<char>()[ValueStart] = '\0';// + Value.size()] = '\0';
  return PDMD;
}

PragmaVoteDecl *
PragmaVoteDecl::CreateDeserialized(ASTContext &C, unsigned ID,
                                             unsigned NameValueSize) {
  return new (C, ID, additionalSizeToAlloc<char>(NameValueSize + 1))
      PragmaVoteDecl(nullptr, SourceLocation(), 0);
}

void PragmaVoteDecl::setVoteType(StringRef Name) {
...
void PragmaVoteDecl::setVoteMethod(StringRef Name) {
  char *Buffer = new (getASTContext(), 1) char[Name.size() + 1];
  memcpy(Buffer, Name.data(), Name.size());
  Buffer[Name.size()] = '\0';
  VoteMethod = Buffer;
...
void PragmaVoteDecl::setMonitorMode(StringRef Name) {
  char *Buffer = new (getASTContext(), 1) char[Name.size() + 1];
  memcpy(Buffer, Name.data(), Name.size());
  Buffer[Name.size()] = '\0';
  MonitorMode = Buffer;
...
void PragmaVoteDecl::setMaxTimeLimit(StringRef Name) {
  char *Buffer = new (getASTContext(), 1) char[Name.size() + 1];
  memcpy(Buffer, Name.data(), Name.size());
  Buffer[Name.size()] = '\0';
  MaxTimeLimit = Buffer;
...
void PragmaVoteDecl::setDepType(std::vector<std::string> Name) {
  //char *Buffer = new (getASTContext(), 1) char[Name[0].size() + 1];//std::vector<std::string> Buffer;// = new (getASTContext(), 1) std::vector<std::string>[Name.size() + 1];
  DepType.assign(Name.begin(), Name.end());//(5, Name.begin());
  //memcpy(Buffer, Name.data(), Name.size());
  //Buffer[Name.size()] = '\0';
  //Dep = Buffer;
}

void PragmaVoteDecl::setDep(std::vector<std::string> Name) {
  //char *Buffer = new (getASTContext(), 1) char[Name[0].size() + 1];//std::vector<std::string> Buffer;// = new (getASTContext(), 1) std::vector<std::string>[Name.size() + 1];
  Dep.assign(Name.begin(), Name.end());//(5, Name.begin());
  //memcpy(Buffer, Name.data(), Name.size());
  //Buffer[Name.size()] = '\0';
  //Dep = Buffer;
}
