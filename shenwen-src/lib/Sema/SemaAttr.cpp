...
// Adding by P
void Sema::ActOnPragmaVote(SourceLocation Loc, StringRef Name, SourceLocation LocEnd, StringRef VoteType,
                                                        SourceLocation LocLBrace, std::vector<std::string> Dep_Var
                                                        , StringRef VoteMethod, std::vector<std::string> Dep_Type
...
//PragmaVoteDecl::setRBraceLoc(LocEnd);
//const auto *PDMD = cast<PragmaVoteDecl>(D);
PDMD->setRBraceLoc(LocEnd);//getName();// setRBraceLoc(LocEnd);
PDMD->setLocStart(Loc);

PDMD->setLBraceLoc(LocLBrace);

PDMD->setVoteType(VoteType);

PDMD->setVoteMethod(VoteMethod);

PDMD->setDep(Dep_Var);

PDMD->setDepType(Dep_Type);
