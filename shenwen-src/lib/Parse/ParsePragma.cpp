...
/// Adding by P
struct PragmaVoteProcedureHandler : public PragmaHandler {
  PragmaVoteProcedureHandler(Sema &Actions)//, Parser &p)
    : PragmaHandler("protect"), Actions(Actions) {}//){}//, parser(p) {}
  void HandlePragma(Preprocessor &PP, PragmaIntroducerKind Introducer,
                    Token &FirstToken) override;
private:
  Sema &Actions;
...
void Parser::initializePragmaHandlers() {
  ...
  // Adding by P
VoteHandler.reset(new PragmaVoteProcedureHandler(Actions));
PP.AddPragmaHandler(VoteHandler.get());
...
void Parser::resetPragmaHandlers() {
  ...
  // Adding by P
  PP.RemovePragmaHandler(VoteHandler.get());
  VoteHandler.reset();
}
...
// Adding by P
void PragmaVoteProcedureHandler::HandlePragma(Preprocessor &PP,
                                            PragmaIntroducerKind Introducer,
                                            Token &Tok) {


  // FIXME: Should we be expanding macros here? My guess is no.
  SourceLocation UnusedLoc = Tok.getLocation();

SourceRange loc;
  loc.setBegin(Tok.getLocation()); 
 ...
 StringRef VoteMethod;


 std::string MonitorMode;
 std::string MaxTimeLimit;


  PP.Lex(Tok);
  const auto *KIND = Tok.getIdentifierInfo();

if (KIND && KIND->isStr("vote")){
 llvm::errs() << KIND->getName() << " vote set.\n";
  //}

PP.Lex(Tok);
 if (!Tok.isNot(tok::identifier)) {
         VoteMethod = Tok.getIdentifierInfo()->getName();
 }
...
//Token Tok;
  ...
  if (Tok.isNot(tok::l_paren)) {
    PP.Diag(Tok.getLocation(), diag::warn_pragma_expected_lparen) << "vote";
    return;
  }
...
// Read the name to embed, which must be a string literal.
 ...
 llvm::errs() << NameString << " NameString.\n";

  //PP.Lex(Tok);

  if (Tok.isNot(tok::comma)) {
    PP.Diag(Tok.getLocation(), diag::err_expected) << tok::comma;
    return;
  }
...
//  PP.Lex(Tok);
  ...
  // Read the comma followed by a second string literal.
  if (!PP.LexStringLiteral(Tok, ValueString, "pragma vote",
                           /*MacroExpansion=*/true))
    return;
 
 llvm::errs() << ValueString << " ValueString.\n";
...
else if (KIND && KIND->isStr("monitor")){
 llvm::errs() << KIND->getName() << " monitor set.\n";
  //}
...
//Token Tok;
  ...
  if (Tok.isNot(tok::l_paren)) {
    PP.Diag(Tok.getLocation(), diag::warn_pragma_expected_lparen) << "monitor";
    return;
  }

// Read the name to embed, which must be a string literal.
 if (!PP.LexStringLiteral(Tok, MonitorMode,
                           "pragma monitor",
                           /*MacroExpansion=*/true))
 return;

 llvm::errs() << MonitorMode << " MonitorMode.\n";
...
//  PP.Lex(Tok);
  ...
  // Read the comma followed by a second string literal.
  if (!PP.LexStringLiteral(Tok, MaxTimeLimit, "pragma monitor",
                           /*MacroExpansion=*/true))
    return;
 
 llvm::errs() << MaxTimeLimit << " MaxTimeLimit.\n";
...
} //else if (II && II->isStr("monitor")){



else if (KIND && KIND->isStr("monitor_vote")){
 llvm::errs() << KIND->getName() << " monitor_vote set.\n";
  //}
...
//Token Tok;
  ...
  if (Tok.isNot(tok::l_paren)) {
    PP.Diag(Tok.getLocation(), diag::warn_pragma_expected_lparen) << "monitor_vote";
    return;
  }
...
//  PP.Lex(Tok);
  ...
  // Read the comma followed by a second string literal.
  if (!PP.LexStringLiteral(Tok, ValueString, "pragma monitor",
                           /*MacroExpansion=*/true))
    return;
...
const auto *SETTWO = Tok.getIdentifierInfo();
  if (SETTWO && SETTWO->isStr("monitor")){
 llvm::errs() << SETTWO->getName() << " SETTWO->monitor.\n";
  }
...
} //else if (II && II->isStr("monitor_vote")){



std::vector<std::string> Dep_Var2;//<StringRef> Dep_Var;//Record_check;
SmallVector<Token, 16> Dep_Var;
std::string Dep_VarString;
std::vector<std::string> Dep_Type;


  if (Tok.is(tok::comma)) {//Not(tok::comma)) {
/*    PP.Diag(Tok.getLocation(), diag::err_expected) << tok::comma;
    return;
  }
*/
 ...
 const IdentifierInfo *arg_dep = Tok.getIdentifierInfo();
 //std::string ar = Tok.getIdentifierInfo()->getName();
  const auto *II = Tok.getIdentifierInfo();
  if (II && II->isStr("dep")){//(getIdentifier(Tok) == ("dep")) {//arg_dep->isStr("dep")) {
  /*  PP.Diag(Tok.getLocation(), diag::err_expected) << "dep";
    return;
  }
  */
 llvm::errs() << II->getName() << " II->isStr dep.\n";
  }
/*
  PP.Lex(Tok);
...
Dep_VarString.clear();
if (!PP.LexStringLiteral(Tok, Dep_VarString, "pragma vote",
                           /*MacroExpansion=*/true))
    return;

 Dep_Var2.push_back(Dep_VarString);
 llvm::errs() <<  "\t" << Dep_Var2[i_dv] << "\t" << " Dep_Var2.\n";
 //i_dv = i_dv + 1;
/*
      PP.Lex(Tok);

      if (Tok.is(tok::identifier)) {
         Dep_Var.push_back(Tok);
         Dep_Var2.push_back(Tok.getIdentifierInfo()->getName());


 llvm::errs() << Tok.getIdentifierInfo()->getName() << "\t" << Dep_Var.size()<< "\t" << Dep_Var.begin() << " Dep_Var.\n";
 llvm::errs() <<  "\t" << Dep_Var2[0] << "\t" << " Dep_Var2.\n";

       // LexID = false;
       // continue;
      }
      else{
            PP.Diag(Tok.getLocation(), diag::err_expected) << " Dep_Var.\n";
            return;        
      }
*/
...
if (!PP.LexStringLiteral(Tok, Dep_VarString, "pragma vote",
  ...
  Dep_Type.push_back(Dep_VarString);
 llvm::errs() <<  "\t" << Dep_Type[i_dv] << "\t" << " Dep_Type.\n";
 i_dv = i_dv + 1;
...
/*
      ...
      if (Tok.is(tok::identifier)) {
         //Dep_Var.push_back(Tok);
         Dep_Type.push_back(Tok.getIdentifierInfo()->getName());


 //llvm::errs() << Tok.getIdentifierInfo()->getName() << "\t" << Dep_Var.size()<< "\t" << Dep_Var.begin() << " Dep_Var.\n";
 llvm::errs() <<  "\t" << Dep_Type[0] << "\t" << " Dep_Type.\n";

        //LexID = false;
        //continue;
      }
      else{
            PP.Diag(Tok.getLocation(), diag::err_expected) << " Dep_Type.\n";
            return;        
      }
*/
      ...
      // Illegal token!
      PP.Diag(Tok.getLocation(), diag::warn_pragma_unused_expected_var);      return;
    }
...
/*
auto Toks = llvm::make_unique<Token[]>(Dep_Var.size());
if(!Dep_Var.empty()){
     std::copy(Dep_Var.begin(), Dep_Var.end(), Toks.get());
}
*/

/*
MutableArrayRef<Token> Toks(
      PP.getPreprocessorAllocator().Allocate<Token>(2 * Dep_Var.size()),
      2 * Dep_Var.size());
  for (unsigned i=0; i != Dep_Var.size(); i++) {
    Token &idTok = Toks[2*i+1];
 //    idTok = Dep_Var[i];
  }
*/
  ...
  if (Tok.isNot(tok::eod)) {
    PP.Diag(Tok.getLocation(), diag::warn_pragma_extra_tokens_at_eol) <<
        "vote:: "<< Tok.getIdentifierInfo()->getName();
    return;
  }
...
if (Tok.isNot(tok::l_brace)) {
    PP.Diag(Tok.getLocation(), diag::err_expected) << tok::l_brace;
    return;
}
else{
  count_stack.push(1);
...
//UnusedLoc=Tok.getLocation();//.setLocation(Tok.getLocation());

loc.setEnd(Tok.getLocation()); 


    // This relies on the loop form is already checked by Sema.
    //Stmt *Body = cast<Stmt *>(RLoc);//getAssociatedStmt()->IgnoreContainers(true);
    //RLoc = Body->getLocEnd();ActOnPragmaVote
...
// The body is a block scope like in Lambdas and Blocks.
      ...
      // Parse statement
      //AssociatedStmt = ParseStatement(RLoc,true);

    sema::FunctionScopeInfo *CurFunction = Actions.getCurFunction();
//      llvm::errs() << CurFunction << " FunctionScopeInfo.\n";

    
    //sema::CompoundScopeInfo *CompoundFunction = getCurCompoundScope();
    //UnusedLoc = CurFunction.get

    //Stmt *CurStmt = Actions->IgnoreContainers(/* IgnoreCaptured */ true);

    Stmt *Pattern = nullptr;
    //Pattern = CurFunction->getBody(CurFunction);

    //UnusedLoc = Actions.getStmt();//getCurFunction().getCurScope().GetEndLoc();
    // getSourceRange();//getLocEnd();//getStmtClass();
    // getCurFunction()

      Actions.ActOnFinishOfCompoundStmt();

//      UnusedLoc = AssociatedStmt.getEnd();


  // Verify that we have a location for the right parenthesis.
//  assert(RParenLoc.isValid() && "Valid '#pragma vote' must have ')'");
//  assert(!Identifiers.empty() && "Valid '#pragma vote' must have arguments");
...
//  PP.EnterTokenStream(Toks, /*DisableMacroExpansion=*/true);
  ...
  // If the pragma is lexically sound, notify any interested PPCallbacks.
  if (PP.getPPCallbacks())
    PP.getPPCallbacks()->PragmaVote(UnusedLoc, NameString, RLoc, ValueString);//,
                                              //ValueString);

  Actions.ActOnPragmaVote(UnusedLoc, NameString, RLoc, ValueString, LLoc, Dep_Var2, VoteMethod, Dep_Type, 
                          MonitorMode, MaxTimeLimit);//Toks);//Actions.ActOnPragmaDetectMismatch(DetectMismatchLoc, NameString, ValueString);
...
/*
  ...
  Token FirstTok;
  FirstTok.startToken();
  FirstTok.setKind(tok::annot_pragma_openmp);
  FirstTok.setLocation(Tok.getLocation());

  while (FirstTok.isNot(tok::eod)) {
    Pragma.push_back(FirstTok);
    PP.Lex(FirstTok);
  }
  SourceLocation EodLoc = FirstTok.getLocation();
  FirstTok.startToken();
  FirstTok.setKind(tok::annot_pragma_openmp_end);
  FirstTok.setLocation(EodLoc);
  Pragma.push_back(FirstTok);
...
//                      /*DisableMacroExpansion=*/false);
  ...
  if (Tok.isNot(tok::identifier)) {
    PP.Diag(Tok.getLocation(), diag::warn_pragma_expected_identifier) <<
          "P1----------vote " ;//<< Tok.getIdentifierInfo()->getName() ;
//    return;
  }
  ...
  if (funcName == StringRef()) {
    PP.Diag(Tok.getLocation(), diag::warn_pragma_expected_identifier) <<
          "P2~~~123 vote " <<funcName<<" ~~~ " << StringRef();
//    return;
  }
  else{
    PP.Diag(Tok.getLocation(), diag::warn_pragma_expected_identifier) <<
          "P3~~~ vote " <<funcName<<" ~~~ " << StringRef();
  }

  Actions.ActOnPragmaVoteProcedure(funcName);
