...
// Adding by P
void Sema::ActOnPragmaVote(SourceLocation Loc, StringRef Name,
                           SourceLocation LocEnd, SourceLocation LocLBrace,
                           StringRef VoteMethod,
                           std::vector<std::string> Target, std::vector<std::string> TargetType,
                           StringRef MonitorMode, StringRef MaxTimeLimit,
                           std::vector<std::string> Dep, std::vector<std::string> DepType)
{
  auto *PDMD = PragmaVoteDecl::Create(
      Context, Context.getTranslationUnitDecl(), Loc, Name);
  Context.getTranslationUnitDecl()->addDecl(PDMD);
  Consumer.HandleTopLevelDecl(DeclGroupRef(PDMD));

  PDMD->setRBraceLoc(LocEnd);
  PDMD->setLocStart(Loc);
  PDMD->setLBraceLoc(LocLBrace);

  // VoteMethod
  PDMD->setVoteMethod(VoteMethod);

  // Target
  PDMD->setTarget(Target);
  PDMD->setTargetType(TargetType);

  // Monitor
  PDMD->setMonitorMode(MonitorMode);
  PDMD->setMaxTimeLimit(MaxTimeLimit);

  // Dep
  PDMD->setDep(Dep);
  PDMD->setDepType(DepType);
