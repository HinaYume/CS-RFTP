...
// Adding by P
void Sema::ActOnPragmaVoteProcedure(StringRef funcName) {
  IsVoteProcedure = true;
}

void Sema::ActOnPendingVotePragma(FunctionDecl* FD) {
  if(IsVoteProcedure) {
    FD->setIsVoteFunction(true);
    IsVoteProcedure = false;
  }
}
...
Sema::ActOnStartOfFunctionDef(Scope *FnBodyScope, Declarator &D,
  ...
  /// Adding by P
  ActOnPendingVotePragma(DP->getAsFunction());
