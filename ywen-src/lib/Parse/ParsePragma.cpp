...
void PragmaVoteProcedureHandler::HandlePragma(Preprocessor &PP,
  ...
  StringRef IdentifierTarget;
  StringRef IdentifierMonitor;
  StringRef IdentifierDep;

  std::string NameString;

  std::vector<std::string> TargetVar;
  std::vector<std::string> TargetVarType;
  ...
  SmallVector<Token, 4> StrName;
  StrName.push_back(Tok);
  StringLiteralParser Literal(StrName, PP);
  NameString = Literal.GetString();
  
  if (NameString.compare("MonitorVoting") == 0) {

    llvm::errs() << "******************************\n";
    llvm::errs() << NameString << "\t\tName read\n";
    ...
    const auto *IVM = Tok.getIdentifierInfo();
    llvm::errs() << IVM->getName() << "\t\tIdentifier read\n";

    PP.Lex(Tok);

    if (!Tok.isNot(tok::identifier)) {
      IdentifierTarget = Tok.getIdentifierInfo()->getName();
    }

    const auto *IT = Tok.getIdentifierInfo();

    if (IT && IT->isStr("target")) {
      llvm::errs() << IT->getName() << "\t\t\tIdentifier read\n\n";
      llvm::errs() << "\33\[1;33m"<< IT->getName() << " variables\tinfo\33\[0m\n";
    }

    unsigned int i_tv = 0;
    unsigned int ParenFlag = 0;
    ...
    while (true) {
      ...
      if (ParenFlag == 1) {
        ...
        TargetString = Literal.GetString();

        TargetVar.push_back(TargetString);
        llvm::errs() << "TargetVar" << ":\t\t" << TargetVar[0] << "\n";
        ...
        if (!PP.LexStringLiteral(Tok, TargetString, "target type",
                                 true))
          return;

        TargetVarType.push_back(TargetString);
        llvm::errs() << "TargetType:\t\t" << TargetVarType[0] << "\n";
        ...
        TargetString.clear();
        if(!PP.LexStringLiteral(Tok, TargetString, "target variables name",
                                true))
          return;

        TargetVar.push_back(TargetString);
        llvm::errs() << "TargetVar" << i_tv+1 << ":\t\t" << TargetVar[i_tv] << "\n";
        ...
        TargetString.clear();
        if(!PP.LexStringLiteral(Tok, TargetString, "target variables type",
                                true))
          return;

        TargetVarType.push_back(TargetString);
        llvm::errs() << "TargetType:\t\t" << TargetVarType[i_tv] << "\n";
    ...
    PP.Lex(Tok);
    const auto *IM = Tok.getIdentifierInfo();

    if (IM && IM->isStr("monitor")) {
      llvm::errs() << "\n" << IM->getName() << "\t\t\tIdentifier read\n";
      llvm::errs() << "\33\[1;33mMonitor details\t\tinfo.\33\[0m\n";
    }
    ...
    if (!PP.LexStringLiteral(Tok, MaxTimeLimit, "monitor MaxTimeLimit",
                             true))
      return;
    ...
    if (Tok.is(tok::comma)) {
      // Read the comma followed by a second string literal.
      if (!PP.LexStringLiteral(Tok, MonitorMode, "monitor MonitorMode",
                               true))
        return;
  ...
  std::vector<std::string> Dep_Var2;
  ...
  std::vector<std::string> Dep_Type;
    ...
    if (II && II->isStr("dep")) { 
      llvm::errs() << II->getName() << "\t\t\tIdentifier read\n";
      llvm::errs() << "\33\[1;33m"<< II->getName() << " variables\t\tinfo.\33\[0m\n";
    }
    ...
    while (true) {
      ...
      if (LexID) {
        ...
        llvm::errs() << "Dep_Var" << i_dv+1 << ":\t\t" << Dep_Var2[i_dv] << "\n";
        ...
        llvm::errs() << "Dep_Type:\t\t" << Dep_Type[i_dv] << "\n";
  ...
  // Stmt *Body = cast<Stmt
  // *>(RLoc);//getAssociatedStmt()->IgnoreContainers(true);
  ...
  if (PP.getPPCallbacks())
    PP.getPPCallbacks()->PragmaVote(UnusedLoc, NameString, RLoc);

  Actions.ActOnPragmaVote(
      UnusedLoc, NameString,
      RLoc, LLoc,
      VoteMethod,
      TargetVar, TargetVarType,
      MonitorMode, MaxTimeLimit,
      Dep_Var2, Dep_Type);
}
