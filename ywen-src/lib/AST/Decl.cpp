...
PragmaVoteDecl *
PragmaVoteDecl::Create(const ASTContext &C, TranslationUnitDecl *DC, SourceLocation Loc,
                       StringRef Name) {
  PragmaVoteDecl *PDMD =
      new (C, DC, additionalSizeToAlloc<char>(Name.size() + 1))
          PragmaVoteDecl(DC, Loc);
  memcpy(PDMD->getTrailingObjects<char>(), Name.data(), Name.size());
  PDMD->getTrailingObjects<char>()[Name.size()] = '\0';
...
PragmaVoteDecl::CreateDeserialized(ASTContext &C, unsigned ID,
                                             unsigned NameSize) {
  return new (C, ID, additionalSizeToAlloc<char>(NameSize + 1))
      PragmaVoteDecl(nullptr, SourceLocation());
}
...
// Target
void PragmaVoteDecl::setTarget(std::vector<std::string> Name) {
  Target.assign(Name.begin(), Name.end());
}

void PragmaVoteDecl::setTargetType(std::vector<std::string> Name) {
  TargetType.assign(Name.begin(), Name.end());
}
...
void PragmaVoteDecl::setDep(std::vector<std::string> Name) {
  Dep.assign(Name.begin(), Name.end());
}

void PragmaVoteDecl::setDepType(std::vector<std::string> Name) {
  DepType.assign(Name.begin(), Name.end());
}
