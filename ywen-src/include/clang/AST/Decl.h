...
// Adding by P
class PragmaVoteDecl final
    : public Decl,
      private llvm::TrailingObjects<PragmaVoteDecl, char> {
  virtual void anchor();
  ...
  PragmaVoteDecl(TranslationUnitDecl *TU, SourceLocation Loc,
                 SourceLocation StartL = SourceLocation(), SourceLocation RBraL= SourceLocation(), SourceLocation LBraL= SourceLocation())
      : Decl(PragmaVote, TU, Loc), LocStart(StartL), RBraceLoc(RBraL), LBraceLoc(LBraL) {}

public:
  static PragmaVoteDecl *Create(const ASTContext &C,
                                TranslationUnitDecl *DC,
                                SourceLocation Loc,
                                StringRef Name);
  static PragmaVoteDecl *
  CreateDeserialized(ASTContext &C, unsigned ID, unsigned NameSize);

StringRef getName() const { return getTrailingObjects<char>(); }
//StringRef getType() const { return getTrailingObjects<char>() + TypeStart; }

/*
StringRef getVoteType() const { return VoteType; }
void setVoteType(StringRef T);
*/

// VoteMethod
StringRef getVoteMethod() const { return VoteMethod; }
void setVoteMethod(StringRef T); 

// Target
llvm::ArrayRef<std::string> getTarget() const { return Target; }
void setTarget(std::vector<std::string> T);

llvm::ArrayRef<std::string> getTargetType() const { return TargetType; }
void setTargetType(std::vector<std::string> T); 

// Monitor
StringRef getMonitorMode() const { return MonitorMode; }
void setMonitorMode(StringRef T); 

StringRef getMaxTimeLimit() const { return MaxTimeLimit; }
void setMaxTimeLimit(StringRef T); 

// Dep
llvm::ArrayRef<std::string> getDep() const { return Dep; }
void setDep(std::vector<std::string> T);

llvm::ArrayRef<std::string> getDepType() const { return DepType; }
void setDepType(std::vector<std::string> T);

  SourceRange getSourceRange() const override LLVM_READONLY {
    return SourceRange(LocStart, RBraceLoc);
  }
  SourceLocation getLocStart() const LLVM_READONLY { return LocStart; }
  SourceLocation getRBraceLoc() const LLVM_READONLY { return RBraceLoc; }
  SourceLocation getLBraceLoc() const LLVM_READONLY { return LBraceLoc; }
  ...
  // Implement isa/cast/dyncast/etc.
  static bool classof(const Decl *D) { return classofKind(D->getKind()); }
  static bool classofKind(Kind K) { return K == PragmaVote; }
};
...
protected:
  FunctionDecl(Kind DK, ASTContext &C, DeclContext *DC, SourceLocation StartLoc,
      ...
      : DeclaratorDecl(DK, DC, NameInfo.getLoc(), NameInfo.getName(), T, TInfo,
        ...
        IsVoteFunction(false), // Adding by P
...
bool isVoteFunction() const { return IsVoteFunction; } // Adding by P
void setIsVoteFunction(bool VF=true) { IsVoteFunction = VF; } // Adding by P
